    ...
    l.controls.Drag.ligerExtend(l.core.UIComponent, {
                ...
                g.receive.each(function (i, obj) {
                    ...
                    && pageY > xy.top && pageY < xy.top + receive.height()) {
                        if (!g.receiveEntered[i]) {
                            if (p.enteredIndex != -1) {
                                // Modify HuangJie 2015-1-30，处理父子容器对象的问题
                                // 判定前一个容器是否当前容器的子容器
                                if (($(g.receive[p.enteredIndex]).width() < receive.width()
                                    || $(g.receive[p.enteredIndex]).height() < receive.height())
                                    && (
                                        $(g.receive[p.enteredIndex]).offset().top >= receive.offset().top
                                        && $(g.receive[p.enteredIndex]).offset().top <= receive.offset().top + receive.height()
                                        && $(g.receive[p.enteredIndex]).offset().left >= receive.offset().left
                                        && $(g.receive[p.enteredIndex]).offset().left <= receive.offset().left + receive.width()
                                       )
                                    ) {
                                    return;
                                }
                                g.receiveEntered[p.enteredIndex] = false;
                            }
                            p.enteredIndex = i;
        ...
        _createProxy: function (proxy, e) {
            ...
            if (typeof proxy == 'function') {
                ...
                g.proxy = g.target.clone().css({ position: "absolute", width: g.target.width(), height: g.target.height() })
