(function ($) {
    ...
    // console.log("ROOT:" + _PORTALROOT_GLOBAL);
    if(_PORTALROOT_GLOBAL != undefined && _PORTALROOT_GLOBAL.length>1 && !_PORTALROOT_GLOBAL.endsWith('/')) {
        _PORTALROOT_GLOBAL += '/';
    }

    // 语言对象
    $.Languages = loadJSON("LanguageData_" + getCurrentLanguage(), _PORTALROOT_GLOBAL + "lang/" + getCurrentLanguage() + ".json");

    // 控制器对象
    $.Controller = loadJSON("ControllerData", _PORTALROOT_GLOBAL + "lang/ControllerData.json");

    //公共地址
    $.PublicAddress = loadJSON("PublicAddressData", _PORTALROOT_GLOBAL + "lang/PublicAddressData.json");
            ...
            $.each(obj, function (n, o) {
                ...
                if (o instanceof Array) {
                    ...
                    for (var i = 0; i < languangKeys.length; i++) {
                        if (o[languangKeys[i]]) {
                            o[languangKeys[i]] = $.Lang(o[languangKeys[i]]) == null ? o[languangKeys[i]] : $.Lang(o[languangKeys[i]]);
                        }
                    }
                }
            });
