var GetSelectedBizObjectId = function () {
    var selectedRows = $("input[name=selectRow][type=radio]").filter(function () {
...
var DoInvokeMethod = function (methodName, displayName, bizObjectId, rowData) {
    $.ajax({
        type: "post",
...
var InvokeMethod = function (methodName, displayName, ifConfirm) {
    ...
    if (bizObjectId) {
        ...
        if (ifConfirm) {
            $.ligerDialog.confirm($.Lang("BizQuery.Execute") + "[" + displayName + "]?", function (result) {
                if (result) {
                    DoInvokeMethod(methodName, displayName, bizObjectId, rowData);
                }
            })
        }
        else {
        ...
        //请选择
        // $.H3Dialog.Warn({content: "请选择一项1111"});
        $.H3Dialog.Warn({content: $.Lang("BizQuery.PleaseSelect")});
    }
}
...
function GetColumns() {
    ...
    if (!radioLoaded && typeof (ActionVisible) != "undefined" && ActionVisible) {
        radioLoaded = true;
        if (window.location.href.toLowerCase().indexOf("listmasterdata.aspx") == -1) {
            columns.splice(0, 0, {
                display: "选择",
                width: "40",
...
var LoadData = function () {
    ...
    masterDataGridManager = $("#masterDataGrid").ligerGrid({
                    ...
                    if (this) {
                        ...
                        for (var p in this) {
                            ...
                            if (typeof (row[p]) === "string") {
                                row[p] = row[p] ? row[p].replace(/\</g, "&lt;") : row[p];
                                row[p] = row[p] ? row[p].replace(/\>/g, "&gt;") : row[p];
                                row[p] = row[p] ? row[p].replace(/\"/g, "&quot;") : row[p];
                            }
                            //时间格式
        ...
        onAfterShowData: function () {
            if (GirdColumns) {
                $(".l-grid-header-table td").not(":first").each(function (k, v) {
                    var tdWidth = parseInt($(v).attr("style").split("width:")[1]);
                    //th用户自定宽度少于100px时设置最小宽度100px
                    if (tdWidth <= 100) {
                        $(v).find("div:first").css("width", "100px");
                    } else {
                        $(v).find("div:first").css("width", tdWidth + "px");
                    }
                });
                ...
                $(".l-grid2 .l-grid-body-table tr").each(function (k, v) {
                    $(this).children().each(function (k, v) {
                        $(v).addClass("tdBreak");
                        $(v).children().css("margin", "0 auto");
                        $(v).find("div:first").css({"text-align": "center", "height": "inherit"});
                        var headerWidth = $(".l-grid-header-table td").not(":first").eq(k).css("width");
                        //td用户自定宽度少于100px时设置最小宽度100px
                        if (parseInt(headerWidth) <= 100) {
                            $(v).find("div:first").css("width", "100px");
                        } else {
                            $(v).find("div:first").css("width", headerWidth);
                        }
                    })
                });
                ...
                $(".l-grid-body1 td.l-grid-row-cell-rownumbers").each(function (k, v) {
                    //使表头排序号的高度随td高度改变
                    var numHeight = $(".l-grid-body2 tr").eq(k).height();
                    if (k === 0) {
                        $(v).find("div:first").css({
                            "height": (numHeight + 1) + "px",
                            "line-height": (numHeight + 1) + "px"
                        });
                    } else {
                        $(v).find("div:first").css({"height": numHeight + "px", "line-height": numHeight + "px"});
                    }
                });
            ...
            //nicescroll
            // $("#masterDataGrid .l-grid-body.l-grid-body2.l-scroll").niceScroll({ cursorcolor: "#37ABFD", cursoropacitymax: 0.8, autohidemode:false,cursorwidth:"5px" }).resize();
        },
...
function DocumentReady() {
    ...
    if (typeof (BizQueryActions) !== "undefined" && BizQueryActions) {
        //add by luwei : sort by sortKey
        BizQueryActions.sort(function (b1, b2) {
        ...
        if (_IsInMasterPage) {
            $(BizQueryActions).each(function () {
                var action = "";
                this.ActionCode = $('<div/>').text(this.ActionCode).html();
                this.DisplayName = $('<div/>').text(this.DisplayName).html();
                //执行动作
...
function GetFilterInput(valuesArray, code, type, defaultValue) {
    ...
    if (valuesArray) {
        ...
        for (var i = 0; i < valuesArray.length; i++) {
            var checked = "";
            valuesArray[i].Value = $('<div/>').text(valuesArray[i].Value).html();
            valuesArray[i].Text = $('<div/>').text(valuesArray[i].Text).html();
            switch (type) {
                case 1://下拉框
...
function GetFilterInputText(logicType, code, filterType, defaultValue) {
    // console.log(logicType, code, filterType, defaultValue, 'code')
    var filterInput = '<td>';
    switch (logicType) {
        case "DateTime":
            if (filterType === 2) {
        ...
        case "Decimal": // 整数 小数
            if (filterType === 2) {
                ...
                filterInput += '<input name="' + code + '" type="text" oninput="InitNumber(event)" value="' + defaultValue + '"/>';
            }
            break;
        default:
            defaultValue = $('<div/>').text(defaultValue).html().replace(/\"/g, "&quot;");
