...
function BindAddControlEvent(btn) {
    btn.onclick = function () {
        ...
        //frames['ifrmWebPartList'].location = _SitePath + "PortalService/WebPartList.aspx?pageid=" + _PageId + "&part=" + $(p).attr("part");
        //frames['ifrmWebPartList'].src = _SitePath + "PortalService/WebPartList.aspx?pageid=" + _PageId + "&part=" + $(p).attr("part");
        
        $modal = $("#webpartEditModal").modal("show");
    }
}
...
function BindEditWebPartInstEvent(btn) {
    btn.onclick = function (e) {
        ...
        frames['ifrmWebPartEdit'].location = _SitePath + "PortalService/WebPartInstEdit.aspx?instid=" + instid;
        frames['ifrmWebPartEdit'].src = _SitePath + "PortalService/WebPartInstEdit.aspx?instid=" + instid;
        $modal = $("#webpartEditModal").modal("show");
    }
}
...
function InitEditModel_ControlAdd() {
    ...
    for (var i = 0; i < parts.length; i++) {
        ...
        //span.innerHTML = GetLanguageText("UI_PAGE_ADDWEBPART"); 添加部件
        span.innerHTML = GetLanguageText("添加部件");
...
function InitEditModel_InstManage() {
    ...
    for (var i = 0; i < insts.length; i++) {
        ...
        if ($(inst).find(".tool-button").length > 0)
            continue;
        var div = document.createElement("div");
        div.className = "tool-button drag-handler";
        ...
        span.className = "button button-collapse";
        span.innerHTML = GetLanguageText("UI_PAGE_FOLD");
        div.appendChild(span);
        span.Inst = inst;
        $(span).click(function (e) {
            var _PanelBody = $(this).parents("[ot_webpart_inst]:first").find(".panel-body:first");
            if (_PanelBody.is(".webpart-fold")) {
                $(this).text(GetLanguageText("UI_PAGE_FOLD"));
                _PanelBody.removeClass("webpart-fold");
            } else {
                $(this).text(GetLanguageText("UI_PAGE_COLLAPSE"));
                _PanelBody.addClass("webpart-fold");
            }
