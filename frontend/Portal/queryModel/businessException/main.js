(function (window, angular, $) {
  ...
  app.controller('businessExceptionCtrl', ['$scope', "$rootScope", "$translate", "$compile", "$http", "$timeout", "$state", "$interval", "$filter", "ControllerConfig", "datecalculation", "jq.datables", '$modal', 'item', 'fileNavigator',
      function ($scope, $rootScope, $translate, $compile, $http, $timeout, $state, $interval, $filter, ControllerConfig, datecalculation, jqdatables, $modal, Item, FileNavigator) {
          ...
          $scope.getMyColumns = function () {
              ...
              columns.push({
                  ...
                  "mRender": function (data, type, full) {
                      ...
                      return `<a style='cursor: pointer;' ng-click='toDetailMyBusinessException(${full})' title='${data}'>${data}</a>`;
                  }
              });
              columns.push({
                  "mData": "orgName",
              ...
              columns.push({
                  "mRender": function (data, type, full) {
                      full = JSON.stringify(full);
                      return `
                            <button class='btn btn-sm btn-default' ng-click='updateMyBusinessException(${full})' title='编辑'>
                                <i class='glyphicon glyphicon-edit'></i>
                            </button>
                       
                            <button class='btn btn-sm btn-danger' ng-click='toDeleteBusinessException(${full})' title='删除'>
          ...
          $scope.dtMyFlowOptions = {
              ...
              "fnServerParams": function (aoData) {
                  ...
                  aoData.push(//name的值是传输数据的key，value的值是传输数据的value
                      { "name": "createTimeStart", "value": $filter("date")( $rootScope.searchExceptionStartTimeStart, "yyyy-MM-dd HH:mm:ss") },
                      { "name": "createTimeEnd", "value": $filter("date")( $rootScope.searchExceptionStartTimeEnd ,"yyyy-MM-dd HH:mm:ss") },
              ...
              "initComplete": function (settings, json) {
                  ...
                  filter.unbind("click.DT").bind("click.DT", function () {
                      var myselect = document.getElementById('businessExceptionType');
          ...
          $('#businessExceptionType').bind('keypress', function (event) { 
              if (event.keyCode == "13") { 
               $("#searchBusinessExceptionOrgName").click();
              }
          })
          $('#searchCompany').bind('keypress', function (event) { 
              if (event.keyCode == "13") { 
               $("#searchBusinessExceptionOrgName").click();
              }
          })
          ...
          $scope.createBusinessException = function (data) {
              var AgencyID = "";
              $http({
                  ...
                  var modalInstance = $modal.open({
                      templateUrl: 'createBusinessException.html',    // 指向上面创建的视图
                      controller: 'businessExceptionModalsController',// 初始化模态范围
                      size: "md",
                      resolve: {
                          params: function () {
                              return {
          ...
          /*************公告管理编辑模态框********************/
          $scope.updateMyBusinessException = function (data) {
              //  console.log(data)
              for (var key in data) {
              ...
              $http({
                  ...
                  // 弹出模态框
                  var modalInstance = $modal.open({
                      templateUrl: 'editBusinessException.html',    // 指向上面创建的视图
                      controller: 'businessExceptionModalsController',// 初始化模态范围
                      size: "md",
                      resolve: {
                          params: function () {
          ...
          /*************共享知识删除模态框*****************/
          $scope.toDeleteBusinessException = function (data) {
              ...
              $http({
                  ...
                  // 弹出模态框
                  var modalInstance = $modal.open({
                      templateUrl: 'deleteMyBusinessException.html',    // 指向上面创建的视图
                      controller: 'businessExceptionModalsController',// 初始化模态范围
                      size: "md",
                      resolve: {
                          params: function () {
          ...
          // ***********共享知识的详情页*******************
          $scope.toDetailMyBusinessException = function (data) {
              ...
              $http({
                  ...
                  // 弹出模态框
                  var modalInstance = $modal.open({
                      templateUrl: 'detailBusinessException.html',    // 指向上面创建的视图
                      controller: 'businessExceptionModalsController',// 初始化模态范围
                      size: "md",
                      resolve: {
                          params: function () {
