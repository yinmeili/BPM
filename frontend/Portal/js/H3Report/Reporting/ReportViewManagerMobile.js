﻿$.fn.LoadReportPage = function (options) {
	console.log(options, "LoadReportPage");
    var reportViewManager = new ReportViewManager(options.ReportFiters, options.ReportPage, options.SourceCode, null, options.Ionic, options.PortalRoot,options.ResetBtn);
}
...
var CommonFunction = {
    ...
    ShowNoneItemImg: function ($container) {
    	//update by ouyangsk 报表筛选无结果时展示提示图片
        //var rootcode = $container.context.URL.split('/');
        //var $img = $("<img src='/" + rootcode[3] + "/css/H3Report/img/NoneReport.png' />").css("margin-top", "3%");
        //var rootcode = window.location.origin + '/Portal/Mobile';
        var $img = $("<img src='img/NoneReport.png' />").css("margin-top", "3%");
...
//********************************************   华丽分割线   ********************************************
//报表展示
window.ReportViewManager = function ($filterContainer, $widgetContainer, reportCode, applist, ionic, PortalRoot,ResetBtn) {
...
ReportViewManager.prototype = {
    Init: function (ionic) {
        ...
        CommonFunction.Post(CommonFunction.LoadReportPage,
            ...
            function (data) {
                if (!data.State) {
                    that.$WidgetContainer.html("<div style='text-align: center;font- size: 18px;'>报表不存在!</div>");
                    return;
                }
                ...
                that.ResetBtnRegister();//注册重置按钮事件
    ...
    CreateLayout: function () {
        ...
        for (var i = 0; i < that.ReportPage.ReportWidgets.length; ++i) {
            ...
            //        $col.addClass("col-md-12").css("width","100%");
            //        break;
            ...
            //            that.$WidgetContainer.append($row);
            //        }
            //        $col.addClass("col-md-6").css("width", "100%");
            //        break;
            //}
            //一行一个
            $row = $("<div>").addClass("row");
            ...
            if (layout == _DefaultOptions.ReportLayout.OneColumn) {
                ...
                //移动端报表不需要和PC一致，和屏幕大小关系，只能一行显示一列
    ...
    CreateWidget: function (widget, $col, WidgetCounter, AnotherCounter) {
        ...
        if (this.ReportPage.ReportWidgets.length == 1)
            location = "";
        //end全屏切换
        var $pannel = $("<div style='height:450px;    background-color: transparent; '>").addClass("panel panel-primary widget-messaging mypanel");//.css("width","300px");
        ...
        //----------------------------------导出,全屏 保存为图片-----------------------------------
        var $body = $("<div>").addClass("panel-body").attr("id", widget.ObjectID).css("min-height", "86%").css("max-height", "86%").css("position", "absolute").css("background-color", "#fff").height("100%");
        ...
        //update by ouyangsk 处理移动设备上全屏查看时有"全屏"字样的问题
        ...
        $AFullScreen.unbind("click").bind("click", function () {
            ...
            if ($pannel.hasClass("Full")) {
                ...
                if (widget.WidgetType == _DefaultOptions.WidgetType.Detail) {
                    //设置明细表缩放以后的样式
                    $body.css("max-height", 386 + "px");
                    $pannel.find(".panel-footer-reportview").css("height", "40px");
                    $pannel.find(".dataTables_scrollBody").css("height", "295px");
                }
        ...
        $AFullScreen.append($IFullScreen);
	...
	// 重置按钮绑定事件
    ResetBtnRegister:function(){
    	
    	var $ele=this.$FilterContainer;
    	var that=this;
    	this.ResetBtn.unbind("click.reset").bind("click.reset",function(){
	    	
	    	//input框全部清空
	        if ($ele.find('input').length>0) {
	            $ele.find('input').val('');
				//$ele.find('input').change();
	        }
	        //日期清空显示
	        if ($ele.find('span.showdate').length>0) {
	            $ele.find('span.showdate').text('');
				//$ele.find('span.showdate').trigger("DOMSubtreeModified");
	
	        }
	        if ($ele.find('input.mydatetimepicker').length>0) {
	            $ele.find('input.mydatetimepicker').val('');
				//$ele.find('div.scroll').find('input.mydatetimepicker').change();
	        }
	
	        //单选
	        if ($ele.find('input:radio').length>0) {
	            $ele.find('input:radio').removeAttr('checked');
				//$ele.find('input:radio').change();
	        }
	        // select 多选
	        if($ele.find('div.bootstrap-select').length>0){
	            $ele.find('div.bootstrap-select').find(".filter-option").text("");
	            $ele.find('div.bootstrap-select').find("li").prop("class","");
				//$ele.find('div.bootstrap-select').change();
	        }
	        //选人清空显示
	        if($ele.find(".sheetuserflag").length>0){
	        	 $ele.find('.sheetuserflag').find(".ion-chevron-right").prev("span").html("");
	        	 that.Ionic.$scope.filters=[];
	        }
	        that.FilterManager.filterValues={};
	        that.ReLoadAllReport();
    	});
    }
};
//********************************************   华丽分割线   ********************************************
    ...
    Init: function () {
        ...
        if (this.filterContainer) {
            for (var k = 0; k < this.Filters.length; k++) {
                ...
                if (Keys != "") {
                    var $colTitle = $("<div class=\"menu-bar-left f15\">" + this.Filters[k].DisplayName + "</div><div class=\"menu-bar-right\"></div>");
                    $row.append($colTitle);
                }
                else {
                    var $colTitle = $("<div class=\"menu-bar-left f15\">" + this.Filters[k].DisplayName + "</div><div class=\"menu-bar-right\"></div>");
        ...
        if ($filterControl) {
            switch (filter.FilterType) {
                case _DefaultOptions.FilterType.Numeric:
                    {
                        console.log(filter,'绑定 Change 事件');
                        $("[id='" + filter.ColumnCode + "']").find('input[data-controlkey="Numeric"]').bind("change", [this, filter.ColumnCode], function (e) {
                            ...
                            //update by ouyangsk 与PC端一致，传数值型类型数据传字符串，后台再转为数值
                            //e.data[0].filterValues[e.data[1].toLowerCase()] = [
                            //    inputs[0].value ? parseFloat(inputs[0].value.trim()) : null,
                            //    inputs[1].value ? parseFloat(inputs[1].value.trim()) : null
                            //];
                            e.data[0].filterValues[e.data[1].toLowerCase()] = [
                ...
                case _DefaultOptions.FilterType.DateTime:
                    {
                        $("[id='" + filter.ColumnCode + "']").find('span.showdate').bind("DOMSubtreeModified", [this, filter.ColumnCode], function (e) {
                            ...
                            e.data[0].filterValues[e.data[1].toLowerCase()] = [
                                $(inputs[0]).text() ? $(inputs[0]).text().trim() : null,
                                $(inputs[1]).text() ? $(inputs[1]).text().trim() : null
                            ];
                            if (that.ValidateFilter())
                                e.data[0].OnChange.apply(e.data[0]);
                        });
                ...
                case _DefaultOptions.FilterType.Organization:
                    {
                    	console.log("_DefaultOptions.FilterType.Organization");
                         that.filterContainer.find("[id='" + filter.ColumnCode + "']").find('input[type="hidden"]').bind("change", function (e) {
                            var datas = [];
                            var dastr = $(this).val();
                            var da = [];
                            if (dastr != "") {
                            ...
                            if (da.length > 1) {
                                ...
                                that.filterValues[filter.ColumnCode.toLowerCase()] = datas;
                            } else {
                                if (da == "") {
                                    var ds = [];
                                    that.filterValues[filter.ColumnCode.toLowerCase()] = ds;
                                } else {
                                    that.filterValues[filter.ColumnCode.toLowerCase()] = da;
                                }
                            }
                            if (that.ValidateFilter())
                ...
                case _DefaultOptions.FilterType.MasterData:
                    if ($filterControl.hasClass("selectpicker")) {
                        $filterControl.bind("change", [this, filter.ColumnCode], function (e) {
                            var v = [];
                            v = $(this).val();
                ...
                case _DefaultOptions.FilterType.Boolean:
                    {
                        if ($filterControl.hasClass("mydropdown")) {
                            ...
                            $("[id='" + filter.ColumnCode + "']").find("input[name='radio-group']").bind("change", [this, filter.ColumnCode], function (e) {
                                var inputs = $("[id='" + filter.ColumnCode + "']").find("input:checked");
                                e.data[0].filterValues[e.data[1].toLowerCase()] = []
...
//        	2.本部门
        	...
        	//初始化默认值
        	var di=this.Ionic.$rootScope.departmentInfo;
        	var defaultName="";
        	var dataFilterValue="data-filtervalue";
        	if(filter.FilterValue=="1"&&di){
        		defaultName=di.UserName;
        		dataFilterValue+="=1";
        	}else if(filter.FilterValue=="2"&&di){
        		defaultName=di.DepartmentName;
        		dataFilterValue+="=2";
        	}
            ...
            switch (parseInt(filter.OrganizationType)) {
                case _DefaultOptions.OrganizationType.User:
                	console.log(_DefaultOptions.OrganizationType.User);
                    dataOrgUnitVisible = 'data-OrgUnitVisible="false"';
                    break;
                case _DefaultOptions.OrganizationType.Dept:
                console.log(_DefaultOptions.OrganizationType.Dept);
                    dataUserVisible = 'data-UserVisible="false"';
                    break;
            }
            var $input = '<div class="sheetuserflag menu-bar"  id="' + filter.ColumnCode + '" ng-click="ReportstateUser($event)" ng-model="filters" ' + dataOrgUnitVisible + ' ' + dataUserVisible +' '+dataFilterValue+'  data-scopedata="' + filter.ColumnCode + '" data-datafield="' + filter.ColumnCode + '">' +
                '<div class="menu-bar-left f15">' + filter.DisplayName + '</div>' +
                '<div class="menu-bar-right f14">' +
            //   '{{filters[\'' + filter.ColumnCode + '\'].Name}}' +
            //     '<span ng-repeat="d in filters[\'' + filter.ColumnCode + '\']" ng-if="$index<1">' +
            //     ' <i>{{d.Name}}</i>' +
            //     '</span>' +
            //    '<span ng-if="filters[\'' + filter.ColumnCode + '\'].length>1" ng-show="{{LanJson.isShow}}">' +
            //     '    等{{filters[\'' + filter.ColumnCode + '\'].length}}人' +
            //     '</span>' +
            //    '<span ng-if="filter[\'' + filter.ColumnCode + '\'].length>1" ng-show="{{!LanJson.isShow}}">' +
            //    '{{filters[\'' + filter.ColumnCode + '\'].length}}{{LanJson.personTotal}}' +
            //'</span>' +
            '<span>'+
            	defaultName//默认值
            +'</span><i class="ion-chevron-right f14 ml10"></i>' +
            '</div>' +
            ...
            var $begin = $("<input class='form-control myform-control' style='height:39px!important'>").attr("data-controlkey", "Numeric").val(beginval);
            //限制输入内容只能为数字
            ...
            var $end = $("<input class='form-control myform-control' style='height:39px!important'>").attr("data-controlkey", "Numeric").val(endval);
            //限制输入内容只能为数字
            $end.keyup(function () {
                if (this.value.length == 1) {
                    this.value = this.value.replace(/[^0-9]/g, '');
            ...
            $input = $("<div>").addClass("input-group input-container").attr("id", filter.ColumnCode).css("flex-wrap", "nowrap").css("padding", "0px");
            ////pc时间插件
            //$input.append($("<div class='input-group-addon' style='padding: 6px 8px;''>从</div>"));
            ...
            var $begintime = '<span class="showdate f14" ng-if="beginvalue" ng-bind-html="beginvalue| date:\'yyyy-MM-dd\'"></span ><span class="showdate f14" ng-if="!beginvalue"></span >';
            ...
            var $endttime = '<span class="showdate f14" ng-if="endvalue" ng-bind-html="endvalue| date:\'yyyy-MM-dd\'"></span ><span class="showdate f14" ng-if="!endvalue"></span >';
            // var $endInput = $("<input type='hidden' style='display:none'  ng-value='endvalue'>").attr("data-controlkey", "DateTime").val(endvalue);
            $input.append($enddiv.append($endttime));
            $begindiv.find("span").text(beginvalue);
            $enddiv.find("span").text(endvalue);
            ...
            var select = "<select class=\"selectpicker\" multiple >";
            if (itemValue) {
                var items = itemValue.split(";;");
                for (var i = 0; items && i < items.length; i++) {
                    var item = items[i].split("::");
                    //update by ouyangsk JV10.4按数据字典编码查询，与PC端 一致
            ...
            var options = filter.FilterValue == null || filter.FilterValue == "" ? "" : filter.FilterValue.split(";");
            $input = $("<div>").attr("id", filter.ColumnCode).css("padding", "0px").css("display", "flex");
...
ChartTableManager.prototype = {
    ...
    BuildTable: function () {
            ...
            for (var key in this.ColumnTable) {
                ...
                for (var i = 0; i < this.ColumnTable[key].length; i++) {
                    ...
                    if (key == "ColumnHeaderTableLastLine" || key == "ColumnHeaderTableLastSecondLine" || i == 0 || that.Widget.LinkageReports == null || that.Widget.LinkageReports.length == 0 || (ColumnCounter == 0 && i == this.ColumnTable[key].length - 1)) {
                        $Th = $('<th style="width:' + mytd.ColSpan * 70 + 'px" row="' + RowCounter + '" data-code="' + mytd.Code + '" col="' + ColumnCounter + '" data-columncode="' + TempColumnCode + '" rowspan="' + mytd.RowSpan + '" colspan="' + mytd.ColSpan + '" data-value="' + mytd.Value + '" >').html(mytd.Value == "" ? "--" : mytd.Value);

                    }
                    else {
                        $Th = $('<th  style="width:' + mytd.ColSpan * 70 + 'px" row="' + RowCounter + '" data-code="' + mytd.Code + '" col="' + ColumnCounter + '" class="unittd" data-columncode="' + TempColumnCode + '" rowspan="' + mytd.RowSpan + '" colspan="' + mytd.ColSpan + '" data-value="' + mytd.Value + '" >').html(mytd.Value == "" ? "--" : mytd.Value);
        ...
        if (this.RowTable != null && !$.isEmptyObject(this.RowTable)) {
            for (var RowI = 0; RowI < this.RowTable.length; RowI++) {
                ...
                for (var RowJ = 0; RowJ < this.RowTable[RowI].length; RowJ++) {
                    ...
                    if (that.Widget.LinkageReports == null || that.Widget.LinkageReports.length == 0) {
                    	//update by ouyangsk
                        $Th = $('<th  style="max-width:' + MyRowTd.ColSpan * 67 + 'px"  data-columncode="' + MyRowTd.ColumnCode + '" data-code="' + MyRowTd.Code + '" data-value="' + MyRowTd.Value + '" rowspan="' + MyRowTd.RowSpan + '" colspan="' + MyRowTd.ColSpan + '">').html(MyRowTd.Value == "" ? "--" : MyRowTd.Value);
                    }
                    else {
                    	//update by ouyangsk
                        $Th = $('<th  style="max-width:' + MyRowTd.ColSpan * 67 + 'px"  class="unittd" data-code="' + MyRowTd.Code + '" data-columncode="' + MyRowTd.ColumnCode + '" data-value="' + MyRowTd.Value + '" rowspan="' + MyRowTd.RowSpan + '" colspan="' + MyRowTd.ColSpan + '">').html(MyRowTd.Value == "" ? "--" : MyRowTd.Value);
                        $Th.unbind("click").bind("click", function () {
                            //联动,作为查询条件；
                            that.FilterData = {};
                            var UnitWidget = that.Widget;
        ...
        if (FlagFreezenColumn || FlagFreezenRow) {
            ...
            $FreezenColumnTable.closest(".panel-body").addClass("scrollingAuto");
...
//                     $FreezenOnlyHeaderTable.css("top", ScrollY);
                    ...
                    $FreezenColumnTable.css("transform", "translate3d(0px, "+ScrollY+", 0px)");
                    $FreezenOnlyHeaderTable.css("transform", "translate3d( 0px, "+ScrollY+", 0px)");
...
//                     $FreezenOnlyHeaderTable.css("left", ScrollX);
                    ...
                    $FreezenRowTable.css("transform", "translate3d("+ScrollX+", 0px, 0px)");
                    $FreezenOnlyHeaderTable.css("transform", "translate3d("+ScrollX+", 0px, 0px)");
    
                }
                if(FlagFreezenColumn&&FlagFreezenRow){
                	$FreezenColumnTable.css("transform", "translate3d(0px, "+ScrollY+", 0px)");
                	$FreezenRowTable.css("transform", "translate3d("+ScrollX+", 0px, 0px)");
                    $FreezenOnlyHeaderTable.css("transform", "translate3d("+ScrollX+", "+ScrollY+", 0px)");
                }
...
GridViewManager.prototype = {
    Init: function () {
        ...
        CommonFunction.Post(
           ...
           { FilterData: JSON.stringify(that.FilterData),
               ...
               UnitFilterDataJson: JSON.stringify(that.UnitFilterDataJson), "start": 0, "length": 0 },
           function (data) {
               ...
               if (!$.isEmptyObject(data.ChildCodes)) {
                   ...
                   for (var key in data.ChildCodes) {
                       ...
                       if (!$.isEmptyObject(node.ChildeColumnSummary)) {
                           ...
                           for (var childkey in node.ChildeColumnSummary) {
                               ...
                               var $Th1 = $("<th colspan='1' rowspan='1'>").attr("data-class", "gridview-th").attr("data-field", childnode.Code).attr("data-align", align).text(childnode.DisplayName).attr('title', childnode.DisplayName);//.css("width", width);
                               $Tr1.append($Th1);
                           ...
                           var $Th = $("<th colspan='" + count + "' rowspan='1'>").attr("data-class", "gridview-th").attr("data-field", node.Code).attr("data-align", align).text(node.DisplayName).attr('title', node.DisplayName);//.css("width", width);
                           $Tr.append($Th);

                       }
                       else {
                           var $Th = $("<th colspan='1' rowspan='2'>").attr("data-class", "gridview-th").attr("data-field", node.Code).attr("data-align", align).text(node.DisplayName).attr('title', node.DisplayName);//.css("width", width);
                           $Tr.append($Th);
                   ...
                   for (var i = 0 ; that.SourceColumns != null && i < that.SourceColumns.length; i++) {
                       ...
                       var $Th = $("<th>").attr("data-class", "gridview-th").attr("data-field", column.ColumnCode).attr("data-align", align).text(column.DisplayName).attr('title', column.DisplayName);//.css("width", width);
                       $Tr.append($Th);
               ...
               that.mydatatable = $Table.DataTable({
                   ...
                   "drawCallback": function (settings) {
                   		...
                   		that.$Container.find('.dataTables_scrollBody').addClass("scrollingAuto");
               ...
               that.$Container.find('.dataTables_scrollBody').css("max-height", that.$Container.css("height").replace("px", "") + 25 + 'px').css("height", "295px");
    ...
    FullScreenTrigger: function () {
        ...
        this.$Container.find('.dataTables_scrollBody').css("height", "460px").css("max-height", this.$Container.css("height").replace("px", "") - this.$Container.find(".dataTables_scrollHead").css("height").replace("px", "") - 50 + 'px');
