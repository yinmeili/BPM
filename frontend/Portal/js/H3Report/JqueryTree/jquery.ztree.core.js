...
(function($){
	...
	tools = {
		...
		clone: function (obj) {
			...
			for (var i in obj) {
			    if (typeof obj[i] == "function" && obj[i] == tools.clone)
			    {
			        console.log("clone tools.clone");
			        continue;
			    }
	...
	view = {
				...
				success: function (apps) {
					...
					try {
					    if (!apps || apps.length == 0) {
					        ...
					        console.log("apptree data Error");
						}
					} catch(err) {
						newNodes = err.toString();
					}
					...
					setting.treeObj.trigger(consts.event.ASYNC_SUCCESS, [setting.treeId, node, apps]);
					tools.apply(callback);
				},
		...
		makeNodeIcoClass: function(setting, node) {
			...
			return  "fa  " + node.iconSkin + " " + icoCss.join('_');
			//return consts.className.BUTTON + " fa  " + node.iconSkin +" "+ icoCss.join('_');
		},
