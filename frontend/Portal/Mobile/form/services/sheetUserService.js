formModule.factory('sheetUserService', function (httpService) {
    return {
        //用户
        sheetUser: function () {
            this.id;
        ...
        deleteSelectItem: function (arry, selectItems) {
            if (!arry || arry.length == 0 || arry.length == 0)
                return arry;
        ...
        sheetUserAdapter: function (users, selecFlag) {
            ...
            users.forEach(function (n, i) {
                ...
                sheetUser.DepartmentName = n.orgInfo;
                    // sheetUser.code = decrypt(n.Code);
                    // sheetUser.name = decrypt(n.Text);
                sheetUser.fullname = sheetUser.name + " [" + sheetUser.code + "]";
                sheetUser.Icon = n.Icon;
                sheetUser.canSelect = false;
                if (n.ExtendObject) {
                    sheetUser.ChildrenCount = n.ExtendObject.ChildrenCount ? n.ExtendObject.ChildrenCount: n.ChildrenCount;
                    sheetUser.DepartmentName = n.ExtendObject.DepartmentName? n.ExtendObject.DepartmentName: n.orgInfo;
