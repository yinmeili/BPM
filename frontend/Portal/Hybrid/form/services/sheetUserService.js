formModule.factory('sheetUserService', function (httpService) {
    return {
        ...
        initItems: function (users) {
            ...
            if (users) {
                if (users.constructor == Object) {
                    var tempUser = { id: users.ObjectID, name: users.Name, type: users.type };
                    objs.push(tempUser);
                } else {
                    users.forEach(function (n, i) {
                        var tempUser = { id: n.ObjectID, name: n.Name, type: n.type };
                        objs.push(tempUser);
                    });
                }
            }
        ...
        convertItems: function (items) {
            if (!items) return [];
            if (items.length == 1) {
                var obj = { Code: items[0].id, Name: items[0].name, ContentType: items[0].type };
                return obj;
            } else {
                var objs = [];
                items.forEach(function (n, i) {
                    objs.push({ Code: n.id, Name: n.name, ContentType: n.type });
                });
                return objs;
            }
        }
