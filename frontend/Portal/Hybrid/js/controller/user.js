module.controller('userCtrl', function ($scope, $state, $http, $stateParams, $cordovaCamera, commonJS) {
    ...
    $scope.init = function () {
        if ($stateParams.id == $scope.user.ObjectID) {
            ...
            var url = $scope.setting.serviceUrl + "/GetUserByObjectID?callback=JSON_CALLBACK&userCode=" + $scope.user.Code + "&mobileToken=" + $scope.user.MobileToken + "&targetUserId=" + $stateParams.id;
            $http.jsonp(url)
                .success(function (result) {
                    ...
                    $scope.userInfo.Image = result.MobileUser.ImageUrl ? $scope.setting.tempImageUrl + result.MobileUser.ImageUrl : $scope.defaultImageUrl;
