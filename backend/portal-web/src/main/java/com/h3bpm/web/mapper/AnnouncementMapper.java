...
public interface AnnouncementMapper {
    ...
    @Select("SELECT `id`, `title`, `description`, `create_user_id` `createUserId`, `type`, `link`, `update_user_id` `updateUserId`, `update_time` `updateTime`, `create_time` `createTime`, `start_time` `startTime`, `end_time` `endTime` from `h3bpm`.`ot_announcement` WHERE `id` = #{id}")
    public Announcement getAnnouncementById(@Param("id") String id);

    @Insert({"INSERT INTO `h3bpm`.`ot_announcement` (`id`, `title`, `description`, `type`, `link`, `start_time`, `end_time`, `create_time`, `create_user_id`) VALUES (#{id}, #{title}, #{description}, #{type}, #{link}, #{startTime}, #{endTime}, #{createTime}, #{createUserId})"})
    public void createAnnouncement(Announcement announcement);

    @Update({"UPDATE `h3bpm`.`ot_announcement` SET `title`= #{title}, `description`=#{description}, `type`=#{type}, `link`=#{link}, `start_time`=#{startTime}, `end_time`=#{endTime}, `update_time`=#{updateTime}, `update_user_id`=#{updateUserId} WHERE `id`=#{id}"})
