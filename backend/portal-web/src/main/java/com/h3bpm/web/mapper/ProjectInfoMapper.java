...
public interface ProjectInfoMapper {
	@Insert({ "INSERT INTO `h3bpm`.`ow_project` (`id`, `name`, `desc`, `leader_id`, `leader_name`, `start_time`,  `end_time`) VALUES (#{id}, #{name}, #{desc}, #{leaderId}, #{leaderName}, #{startTime}, #{endTime})" })
	public void createProjectInfo(ProjectInfo projectInfo);

	@Update({ "UPDATE `h3bpm`.`ow_project` SET `name`= #{name}, `desc`=#{desc}, `leader_id`=#{leaderId}, `leader_name`=#{leaderName}, `start_time`=#{startTime}, `end_time`=#{endTime} WHERE `id`=#{id}" })
	...
	public List<ProjectInfo> findProjectInfo(@Param("keyword") String keyword, @Param("leaderId") String leaderId, @Param("startTimeStart") Date startTimeStart, @Param("startTimeEnd") Date startTimeEnd, @Param("endTimeStart") Date endTimeStart, @Param("endTimeEnd") Date endTimeEnd);
